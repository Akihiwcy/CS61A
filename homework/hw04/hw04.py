...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert is_mobile(m), "must call balanced on mobile"
    l_arm, r_arm = left(m), right(m)
    if is_planet(end(l_arm)):
        l_weight = size(end(l_arm))
    else:
        if not balanced(end(l_arm)):
            return False
        l_weight = total_weight(end(l_arm))
    if is_planet(end(r_arm)):
        r_weight = size(end(r_arm))
    else:
        if not balanced(end(r_arm)):
            return False
        r_weight = total_weight(end(r_arm))
    
    return l_weight * length(l_arm) == r_weight * length(r_arm)
