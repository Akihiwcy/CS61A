...
def missing_digits(n):
    ...
    def helper(n, hope):
        if n == 0:
            return 0
        return max(hope - n % 10, 0) + helper(n // 10, n % 10 - 1)
...
def count_change(total):
    ...
    def helper(total, big_coin):
        ...
        if big_coin == 1:
            return 1
        return helper(total - big_coin, big_coin) + helper(total, big_coin // 2)

    def get_big_coin(coin):
        if 2 * coin > total:
            return coin
        return get_big_coin(coin * 2)

    return helper(total, get_big_coin(1))
