...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    typed_without_empty = [w for w in typed_words if w != '']
    reference_without_empty = [w for w in reference_words if w != '']
    idx = 0
    correct = 0
    while idx < len(typed_without_empty) and idx < len(reference_without_empty):
        if typed_without_empty[idx] == reference_without_empty[idx]:
            correct += 1
        idx += 1
    return 100 * (correct / len(typed_without_empty)) if len(typed_without_empty) != 0 else 0.0
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    candidate = min(valid_words, key=lambda w: diff_function(user_word, w, limit))
    return candidate if diff_function(user_word, candidate, limit) <= limit else user_word
    # END PROBLEM 5
...
def meowstake_matches(start, goal, limit):
    ...
    if not goal or not start or limit < 0: # Fill in the condition
        # BEGIN
...
def time_per_word(times_per_player, words):
    ...
    for tpp in times_per_player:
        times.append([tpp[i+1]-tpp[i] for i in range(len(tpp)-1)])
...
def fastest_words(game):
    ...
    for i, w in enumerate(allwords):
        word_time = [times[p][i] for p in players]
        player_idx = min(players, key = lambda x: word_time[x])
        fastest[player_idx].append(w)
    return fastest
    # END PROBLEM 10
