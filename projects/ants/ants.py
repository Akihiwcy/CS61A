...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        p = self.place
        distance = 0
        while p.entrance != beehive and (len(p.bees) == 0 or distance < self.min_range):
            p, distance = p.entrance, distance + 1
        return rANTdom_else_none(p.bees) if self.min_range <= distance <= self.max_range else None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        def reduce_all(amount):
            cp_bees = self.place.bees[:]
            for bee in cp_bees:
                bee.reduce_armor(amount)

        if amount >= self.armor:
            reduce_all(self.damage)
        reduce_all(amount)
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.contained_ant:
            self.contained_ant.action(gamestate=gamestate)
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.identity = self.is_first
        QueenAnt.is_first = False
        self.doubled = set() # Remember the ants that have been doubled
        "*** YOUR CODE HERE ***"
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.identity == False:
            ...
            p = self.place.exit
            while p:
                if p.ant and (p.ant not in self.doubled):
                    p.ant.damage *= 2
                    self.doubled.add(p.ant)
                if isinstance(p.ant, ContainerAnt) and p.ant.contained_ant and p.ant.contained_ant not in self.doubled:
                    p.ant.contained_ant.damage *= 2
                    self.doubled.add(p.ant.contained_ant)
                p = p.exit
            # Attack by itself
    ...
    def reduce_armor(self, amount):
        ...
        if self.identity and self.armor <= 0:
            bees_win()
        "*** YOUR CODE HERE ***"
        # END Problem 13
