...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        def not_remove(card, other_card):
            return card.attack != other_card.attack and card.defense != other_card.defense
        cp_cards = [card for card in opponent.deck.cards if not_remove(card, other_card)]
        opponent.deck.cards = cp_cards[:]
